...
class VGGPerceptualLoss(nn.Module):
    def __init__(self, resize=False):
        ...
        # Use layers up to relu3_3 (can go deeper if needed)
    ...
    def forward(self, x, y):
        # Make sure inputs are 3-channel and normalized to VGG stats
        # (VGG expects ImageNet normalization: mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        if x.shape[1] == 1:  # grayscale → fake 3 channels
            x = x.repeat(1, 3, 1, 1)
            y = y.repeat(1, 3, 1, 1)
        elif x.shape[1] == 3:
            pass  # already fine
...
class BetaVAE(nn.Module):
    def __init__(self, encoder, latent_dim=16, beta=0.5): #beta=1 is the vanilla vae
    ...
    def loss_function_vgg(self, recon_x, x, mu, log_var):
        perceptual_loss_fn = VGGPerceptualLoss()

        mse_loss = F.mse_loss(recon_x, x, reduction='sum')
        perp_loss = perceptual_loss_fn(recon_x, x)   # feature-based loss
        KL_div = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
        loss = 0.7 * perp_loss + 0.2 * mse_loss + 0.1 * self.tv_loss(recon_x) + self.beta * KL_div
        return loss

    def loss_function_mix(self, recon_x, x, mu, log_var):
        perceptual_loss_fn = VGGPerceptualLoss()

        recon_loss = perceptual_loss_fn(recon_x, x)   # feature-based loss
        KL_div = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
...
def train():
  ...
  vae = BetaVAE(encoder, latent_dim=16, beta=0.01)
  ...
  for epoch in range(num_epochs):
      ...
      for x, labels in dataloader:
          ...
          loss = vae.loss_function_mix(reconstructed_x, x, mu, log_var)
          loss.backward()
          optimizer.step()
