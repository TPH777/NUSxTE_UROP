...
# If you previously had `import torch` and removed it, just ensure it appears once.

# (Optional: silence tokenizer parallel warning)
...
def main():
    ...
    if args.precompute_text_embeddings:
        with torch.no_grad():
            ...
            for img_name in tqdm([i for i in os.listdir(args.data_config.control_dir) if i.lower().endswith(('.png', '.jpg', '.jpeg'))]):
                ...
                if not os.path.isfile(txt_path):
                    print(f"[warn] missing caption file {txt_path}, skipping")
                    continue
                ...
                if args.save_cache_on_disk:
                    torch.save(record, os.path.join(txt_cache_dir, txt_key + '.pt'))
                else:
                    cached_text_embeddings[txt_key] = record

                # empty (dropout) embedding
                ...
                empty_record = {
                    'prompt_embeds': empty_embeds[0].to('cpu'),
                    'prompt_embeds_mask': empty_mask[0].to('cpu')
                }
                empty_key = txt_key + 'empty_embedding'
                if args.save_cache_on_disk:
                    torch.save(empty_record, os.path.join(txt_cache_dir, empty_key + '.pt'))
                else:
                    cached_text_embeddings[empty_key] = empty_record
    ...
    train_dataloader = loader(
        ...
        cached_image_embeddings_control=cached_image_embeddings_control,  # revert: this matches CustomImageDataset
        **args.data_config
    )
    ...
    # Compute VAE downsample (spatial) scale factor robustly.
    # Old (removed): vae_scale_factor = 2 ** (len(vae.config.block_out_channels) - 1)
    cfg = vae.config
    block_out_channels = None
    try:
        if isinstance(cfg, dict):
            block_out_channels = cfg.get("block_out_channels", None)
        else:
            # FrozenDict supports get
            block_out_channels = cfg.get("block_out_channels", None)
    except Exception:
        block_out_channels = None
    ...
    if block_out_channels:
        ...
        # Probe dynamically (safe & accurate)
        try:
            import torch
            with torch.no_grad():
                test_h = 256
                dummy = torch.zeros(1, 3, test_h, test_h, device=vae.device, dtype=vae.dtype)
                latents = vae.encode(dummy).latent_dist.sample()
                vae_scale_factor = test_h // latents.shape[-1]
                # sanity clamp
                if vae_scale_factor not in (4, 8, 16, 32):
                    vae_scale_factor = 8
        except Exception:
            vae_scale_factor = 8  # fallback default for SD-style VAEs
    print(f"[info] Using vae_scale_factor={vae_scale_factor}")
